...
type FieldDefinition = { type: string; refTable?: string; optional?: boolean; itemType?: any; flexible?: boolean };
...
class TableBuilder {
    ...
    private buildFields(fields: TableDefinition) {
        for (const [name, field] of Object.entries(fields)) {
            ...
            if (field.type === "array" && field.itemType) {
                type = `array<${field.itemType.type}>`;
            }

            const ref = field.refTable ? ` REFERENCES TABLE ${field.refTable}` : "";
            const flexibleType = field.flexible ? "FLEXIBLE TYPE object" : type;
            this.fields.push(`DEFINE FIELD ${name} ON TABLE ${this.table} TYPE ${field.type === "object" ? flexibleType : type} ${ref};`);
        }
    }
     ...
     *     .generate();
     */
    index(indexName: string, fields: string | [string], unique: boolean = false): this {
        const fieldList = Array.isArray(fields) ? fields.join(", ") : fields;
        const uniqueStr = unique ? " UNIQUE" : "";
        this.indexes.push(`DEFINE INDEX ${indexName} ON TABLE ${this.table} FIELDS ${fieldList} ${uniqueStr};`);
        return this;
    }
     ...
     */
    generate(): string {
        return `DEFINE TABLE IF NOT EXISTS ${this.table} ${this.type}; 
                ${this.fields.join("\n")}
                ${this.indexes.join("\n")}
                `;
    }
 ...
 * console.log(query);
 */
export function defineSchema(table: string, schemaType: TABLETYPE = TABLETYPE.SCHEMALESS, fields: TableDefinition): TableBuilder {
